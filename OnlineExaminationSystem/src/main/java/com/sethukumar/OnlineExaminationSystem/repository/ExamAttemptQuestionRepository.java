...
public interface ExamAttemptQuestionRepository
    ...
    List<ExamAttemptQuestion> findByAttemptId(Long attemptId);

    @Query("""
                SELECT aq.question, AVG(aq.score)
                FROM ExamAttemptQuestion aq
                WHERE aq.score IS NOT NULL
    ...
    @Query("""
                SELECT new com.sethukumar.OnlineExaminationSystem.dto.ScorePerQuestionDTO(
                    q.id, q.text, q.maxMarks, aq.score, q.type
                )
                FROM ExamAttemptQuestion aq JOIN aq.question q
                WHERE aq.attempt.id = :attemptId
            """)
    List<ScorePerQuestionDTO> findScoresByAttempt(@Param("attemptId") Long attemptId);
}
