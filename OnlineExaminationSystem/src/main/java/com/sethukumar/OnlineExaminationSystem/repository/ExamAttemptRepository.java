...
public interface ExamAttemptRepository extends JpaRepository<ExamAttempt, Long> {
    ...
    List<ExamAttempt> findByExamIdAndGradedFalse(Long examId);

    @Query("""
                SELECT a.student FROM ExamAttempt a
                WHERE a.exam.id = :examId
                  AND a.totalScore = (
                      SELECT MAX(a2.totalScore) FROM ExamAttempt a2
                      WHERE a2.exam.id = :examId AND a2.graded = true
                  )
                  AND a.graded = true
            """)
    List<User> findHighestScoringStudents(@Param("examId") Long examId);
    ...
    @Query("""
            ...
            """)
    List<User> findFailedStudents(@Param("examId") Long examId, @Param("passMark") int passMark);
    ...
    @Query("""
                SELECT new com.sethukumar.OnlineExaminationSystem.dto.ExamResultDTO(
                    s.id, s.name, a.totalScore
                )
                FROM ExamAttempt a JOIN a.student s
                WHERE a.exam.id = :examId AND a.graded = true
            """)
    List<ExamResultDTO> findResultsByExam(@Param("examId") Long examId);
}
